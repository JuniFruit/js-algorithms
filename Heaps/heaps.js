...
export class MinHeap {
  ...
  #canSwap(ind) {
    return (
      (this.#exists(this.#leftChild(ind)) && this.heap[this.#leftChild(ind)] < this.heap[ind]) ||
      (this.#exists(this.#rightChild(ind)) && this.heap[this.#rightChild(ind)] < this.heap[ind])
    );
  }
  ...
  #heapifyDown() {
    ...
    while (this.#canSwap(currentInd)) {
      ...
      if (this.#exists(rightChild) && this.heap[leftChild] > this.heap[rightChild]) {
        // prettier-ignore
...
export class MinHeapWithComparator {
  constructor() {
    this.heap = [null];
    this.n = 0;
  }
  ...
  #bubbleUp() {
    let curr = this.n;
    while (this.heap[curr] && this.heap[curr].compareTo(this.heap[this.#parent(curr)]) < 0) {
      [this.heap[curr], this.heap[this.#parent(curr)]] = [
        this.heap[this.#parent(curr)],
        this.heap[curr],
      ];
  ...
  #heapifyDown(start = 1) {
    ...
    while (this.#canSwap(currentInd)) {
      let leftChild = this.#leftChild(currentInd);
      let rightChild = this.#rightChild(currentInd);
      if (this.#exists(rightChild) && this.heap[leftChild].compareTo(this.heap[rightChild]) > 0) {
        // prettier-ignore
        [this.heap[rightChild], this.heap[currentInd]] = [this.heap[currentInd],this.heap[rightChild]];
        currentInd = rightChild;
      } else {
